...
########Plotting the line contours of IMD max temp for any one day########
...
levels=np.linspace(temp.min(),temp.max(),30)
fig,ax=plt.subplots(1,1)
cp=ax.contour(X,Y,temp[121,:,:],levels=levels)	#	for Line contours
fig.colorbar(cp)
ax.set_title('Maximum Temperature in 2018')
ax.set_xlabel('Longitude')
ax.set_ylabel('Latitude')

plt.savefig('contour_imd_line.png')		
exit()
